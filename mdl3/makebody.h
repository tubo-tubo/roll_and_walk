...
/*
void makebody::sensor()
{

  i = 0;
  dMassSetZero(&mass);
  dMassSetSphere(&mass,1,r_thigh);
  do{
    s_roll[i].body = dBodyCreate(world);
    s_roll[i].geom = dCreateSphere(space,r_thigh);
    s_roll1[i] = dJointCreateFixed(world,0);
    s_roll2[i] = dJointCreateFixed(world,0);
    s_comaneci[i].body = dBodyCreate(world);
    s_comaneci[i].geom = dCreateSphere(space,r_thigh);
    s_comaneci1[i] = dJointCreateFixed(world,0);
    s_comaneci2[i] = dJointCreateFixed(world,0);
    s_knee[i].body = dBodyCreate(world);
    s_knee[i].geom = dCreateSphere(space,r_thigh);
    s_knee1[i] = dJointCreateFixed(world,0);
    s_knee2[i] = dJointCreateFixed(world,0);
    s_ankle[i].body = dBodyCreate(world);
    s_ankle[i].geom = dCreateSphere(space,r_thigh);
    s_ankle1[i] = dJointCreateFixed(world,0);
    s_ankle2[i] = dJointCreateFixed(world,0);
    s_tire[i].body = dBodyCreate(world);
    s_tire[i].geom = dCreateSphere(space,r_thigh);
    s_tire1[i] = dJointCreateFixed(world,0);
    s_tire2[i] = dJointCreateFixed(world,0);

    dBodySetMass(s_roll[i].body,&mass);
    dBodySetMass(s_comaneci[i].body,&mass);
    dBodySetMass(s_knee[i].body,&mass);
    dBodySetMass(s_ankle[i].body,&mass);
    dBodySetMass(s_tire[i].body,&mass);

    dGeomSetBody(s_roll[i].geom,s_roll[i].body);
    dGeomSetBody(s_comaneci[i].geom,s_comaneci[i].body);
    dGeomSetBody(s_knee[i].geom,s_knee[i].body);
    dGeomSetBody(s_ankle[i].geom,s_ankle[i].body);
    dGeomSetBody(s_tire[i].geom,s_tire[i].body);

    dBodySetPosition(s_roll[i].body,roll[i].x0,roll[i].y0,roll[i].z0);
    dBodySetPosition(s_comaneci[i].body,comaneci[i].x0,comaneci[i].y0,comaneci[i].z0);
    dBodySetPosition(s_knee[i].body,knee[i].x0,knee[i].y0,knee[i].z0);
    dBodySetPosition(s_ankle[i].body,ankle[i].x0,ankle[i].y0,ankle[i].z0);
    dBodySetPosition(s_tire[i].body,tire[i].x0,tire[i].y0,tire[i].z0);

    dJointAttach(s_roll1[i],s_roll[i].body,base.body);
    dJointAttach(s_roll2[i],s_roll[i].body,thigh[i].body);
    dJointAttach(s_comaneci1[i],s_comaneci[i].body,roll[i].body);
    dJointAttach(s_comaneci2[i],s_comaneci[i].body,thigh[i].body);
    dJointAttach(s_knee1[i],s_knee[i].body,thigh[i].body);
    dJointAttach(s_knee2[i],s_knee[i].body,shin[i].body);
    dJointAttach(s_ankle1[i],s_ankle[i].body,shin[i].body);
    dJointAttach(s_ankle2[i],s_ankle[i].body,foot[i].body);
    dJointAttach(s_tire1[i],s_tire[i].body,foot[i].body);
    dJointAttach(s_tire2[i],s_tire[i].body,wheel[i].body);

    dJointSetFeedback(s_roll1[i],r_feedback);
    dJointSetFeedback(s_comaneci1[i],c_feedback);
    dJointSetFeedback(s_knee1[i],k_feedback);
    dJointSetFeedback(s_ankle1[i],a_feedback);
    dJointSetFeedback(s_tire1[i],t_feedback);
